...
class Anuncios extends model {
  ...
  public function getUltimosAnuncios($page, $pag,$filtros) {
    ...
    $sql = $this->db->prepare("SELECT*,(select anuncios_img.url from anuncios_img where anuncios_img.id_anuncio = anuncios.id limit 1) as url,(select categoria.nome from categoria where categoria.id = anuncios.id_categoria) as categoria FROM anuncios WHERE ".implode(' AND ',$filtrostring)." ORDER BY id DESC LIMIT $offset,$pag");
  ...
  public function getAnuncio($id) {
  	...
  	$sql = $this->db->prepare("SELECT *,(select categoria.nome from categoria where categoria.id = anuncios.id_categoria) as categoria,(select usuarios.telefone from usuarios where usuarios.id = anuncios.id_usuario) as telefone
     FROM anuncios WHERE id = :id");
      ...
      $sql = $this->db->prepare("SELECT id,url FROM anuncios_img WHERE id_anuncio = :id_anuncio");
    ...
    if(count($fotos) > 0) {
      for ($q=0; $q <count($fotos['tmp_name']) ; $q++) { 
        ...
        if(in_array($tipo, array('image/jpeg','image/png'))) {
          ...
          $sql = $this->db->prepare("INSERT INTO anuncios_img SET id_anuncio = :id_anuncio, url = :url");
          $sql->bindValue(':id_anuncio',$id);
  ...
  public function excluirAnuncio($id) {
  	  ...
  	  $sql =  $this->db->prepare("DELETE FROM anuncios_img WHERE id_anuncio = :id_anuncio");
  	  $sql->bindValue(":id_anuncio", $id);
  	  $sql->execute();
  ...
  public function excluirFoto($id) {
    ...
    $sql = $this->db->prepare("SELECT id_anuncio FROM anuncios_img WHERE id = :id");
      $sql->bindValue(":id", $id);
      $sql->execute();
    ...
    $sql =  $this->db->prepare("DELETE FROM anuncios_img WHERE id = :id");
      $sql->bindValue(":id", $id);
      $sql->execute();
